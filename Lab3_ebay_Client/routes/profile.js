	...
	if(req.session.login)
	{
		...
		soap.createClient(url,option, function(err, client) {
			client.removeItem(args, function(err, result) {
				if(result.removeItemReturn != null)
				{
					response = {"statusCode":200,"data":JSON.parse(result.removeItemReturn)};
					res.send(JSON.stringify(response));
				}
				else
				{
	...
	if(req.session.login)
	{
		...
		soap.createClient(url,option, function(err, client) {
			client.getProfile(args, function(err, result) {
				if(result.getProfileReturn != null)
				{
					response = {"statusCode":200,"data":JSON.parse(result.getProfileReturn)};
					res.send(JSON.stringify(response));
				}
				else
				{
